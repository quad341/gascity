...
// Bug C: convoy control --serve crashed on the first per-bead error, taking
// down the entire dispatcher. The most common case is a workflow-finalize
// bead whose root bead has been deleted. Closing the finalizer with a
// missing-root outcome lets the serve loop continue past the orphan.
func TestProcessWorkflowFinalizeOrphanedRootClosesFinalizerWithoutError(t *testing.T) {
	...
	if !result.Processed {
		t.Fatalf("result = %+v, want processed (orphaned finalizer should still be reaped)", result)
	}
	...
	if finalizerAfter.Status != "closed" {
		t.Fatalf("finalizer status = %q, want closed (must not strand the serve loop)", finalizerAfter.Status)
	}
