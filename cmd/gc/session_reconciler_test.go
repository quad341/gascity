...
func TestPendingCreateSessionStillLeased_StaleBeadWithoutLeaseIsOrphan(t *testing.T) {
	...
	// last_woke_at empty AND was created more than the never-started lease
	...
	bead := beads.Bead{
		CreatedAt: now.Add(-(pendingCreateNeverStartedTimeout + time.Second)),
		Metadata: map[string]string{
			"template":             "worker",
			"state":                "creating",
			"pending_create_claim": "true",
...
func TestPendingCreateSessionStillLeased_RecentBeadWithoutLeaseIsLeased(t *testing.T) {
	...
	// leased while CreatedAt is within the never-started lease window.
	cfg := &config.City{Agents: []config.Agent{{Name: "worker"}}}
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	clk := &clock.Fake{Time: now}
	bead := beads.Bead{
...
func TestPendingCreateSessionStillLeased_ConfigCatchUpGracePreservesFreshBead(t *testing.T) {
	...
	// requirement. The active pending-create lease protects the bead until
	// the config snapshot catches up.
...
func TestPendingCreateSessionStillLeased_ConfigCatchUpExpiresWhenStale(t *testing.T) {
	// The catch-up grace is bounded by the active pending-create lease. Once
	// that lease expires without the agent appearing, the bead is
	// orphan-eligible.
	cfg := &config.City{Agents: []config.Agent{{Name: "other-template"}}}
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	clk := &clock.Fake{Time: now}
