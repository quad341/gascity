...
func processWorkflowFinalize(store beads.Store, bead beads.Bead) (ControlResult, error) {
	...
	if err := setOutcomeAndClose(store, rootID, outcome); err != nil {
		// Bug C: an orphaned finalizer (root deleted out from under it)
		// must not crash the entire serve loop. Close the finalizer with
		// a missing_root outcome so it stops being processed; the serve
		// loop moves on to the next bead.
		if errors.Is(err, beads.ErrNotFound) {
			if closeErr := setOutcomeAndClose(store, bead.ID, "missing_root"); closeErr != nil {
				return ControlResult{}, fmt.Errorf("%s: closing orphaned finalizer (root %s missing): %w", bead.ID, rootID, closeErr)
			}
