...
func newDoctorCmd(stdout, stderr io.Writer) *cobra.Command {
	var fix, verbose, jsonOut, explainPostgresMacOSBootstrap bool
	cmd := &cobra.Command{
		Use:   "doctor",
...
branch.`,
  ...
  gc doctor --explain-postgres-macos-launchd-bootstrap`,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			if explainPostgresMacOSBootstrap {
				if err := writePostgresMacOSBootstrapDoc(stdout); err != nil {
	...
	cmd.Flags().BoolVar(&explainPostgresMacOSBootstrap, "explain-postgres-macos-launchd-bootstrap", false, "print the local PostgreSQL macOS launchd bootstrap runbook")
	return cmd
}
