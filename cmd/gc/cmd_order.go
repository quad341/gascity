...
func cmdOrderHistory(name, rig string, stdout, stderr io.Writer) int {
	...
	return routeOrderHistory(cityPath, cfg, name, rig, aa, c, reason, stdout, stderr)
}
...
func routeOrderHistory(cityPath string, cfg *config.City, name, rig string, aa []orders.Order, c *api.Client, nilReason string, stdout, stderr io.Writer) int {
	...
	if c != nil {
		...
		if err == nil {
			...
			return renderOrderHistoryFromAPI(cr, name, stdout)
		}
...
func renderOrderHistoryFromAPI(cr api.CachedRead[[]api.OrderHistoryView], name string, stdout io.Writer) int {
	entries := cr.Body
	if len(entries) == 0 {
		if name != "" {
